...
def create_dataset_json(dataset_path, modalities, numTraining, file_ending=".nii.gz", pediatric=False):
    """
...
def process_brats2023_for_nnunet(brats_path, nnunet_raw_path, dataset_id, dataset_name='brats', modalities=None, sample=None, verbose=True, pediatric=False):
    """
    ...
    for idx, patient_folder in enumerate(tqdm(patient_folders, desc="Processing patients")):
        ...
        for modality, file_path in dict_file_paths.items():
            try:
                ...
                if modality == "seg":
                    img = nib.Nifti1Image(img.get_fdata().astype(np.uint8), img.affine, img.header)
    ...
    create_dataset_json(dataset_path, modalities, numTraining, file_ending=".nii.gz", pediatric=pediatric)
...
def main():
    ...
    parser.add_argument('--pediatric', action='store_true', help='Include an additional "cystic component" label.')
