...
def create_class_mapping():
    ...
    # Define left-right pairs to be merged
    left_right_pairs = [
        ...
        ("left_inferior_lateral_ventricle", "right_inferior_lateral_ventricle"),
    ...
    # Initialize new class indices
    new_class_index = 0
    class_mapping = {}
    class_names = {}

    # Map background
    class_mapping[0] = new_class_index
    class_names[new_class_index] = "background"
    new_class_index += 1

    # Map left-right pairs to the same class index
    for left_name, right_name in left_right_pairs:
        left_index = original_labels[left_name]
        right_index = original_labels[right_name]
        class_mapping[left_index] = new_class_index
        class_mapping[right_index] = new_class_index
        class_names[new_class_index] = f"{left_name}/{right_name}"
        new_class_index += 1

    # Map unpaired classes individually
    all_left_right_names = set([name for pair in left_right_pairs for name in pair])
    unpaired_classes = [
        name for name in original_labels
        if name not in all_left_right_names and name != 'background'
    ]

    for class_name in unpaired_classes:
        original_index = original_labels[class_name]
        class_mapping[original_index] = new_class_index
        class_names[new_class_index] = class_name
        new_class_index += 1

    return class_mapping, class_names

# Create class mapping and class names
class_mapping, class_names = create_class_mapping()

def encode_image(img_data, class_mapping):
    # Apply the class mapping directly using numpy's vectorize
    vectorized_encode = np.vectorize(lambda x: class_mapping.get(x, 0))
    encoded_img_data = vectorized_encode(img_data)
    return encoded_img_data

def create_dataset_json(dataset_path, modalities, numTraining, class_names, file_ending=".nii.gz"):
    """
    ...
    Args:
        ...
        class_names (dict): Mapping of new class indices to class names.
    ...
    # Create labels mapping from class names to new class indices
    labels = {class_name: new_index for new_index, class_name in class_names.items()}
...
def copy_old_paths_to_new_paths_from_dictionary(dict_file_paths, new_file_paths, class_mapping):
    """
    Copy files from old paths to new paths based on the provided dictionaries.
    If the modality is 'seg', encode the segmentation image using the class mapping before saving.
    
    Args:
        dict_file_paths (dict): Dictionary with original file paths.
        new_file_paths (dict): Dictionary with new file paths.
        class_mapping (dict): Mapping from original class indices to new class indices.
    """
    for modality, old_path in dict_file_paths.items():
        ...
        if modality == 'seg':
            ...
            encoded_img_data = encode_image(img_data, class_mapping)
...
def process_ukb_dataset(synthseg_path, nnunet_raw_path, dataset_id, verbose, modalities, sample=None, dataset_name="synthseg"):
    """
    ...
    # Check if the directories are full and clear them
    ...
    # Remove existing dataset.json if present
    ...
    for ukb_identifier in tqdm(ukb_identifiers, desc="Processing UKB identifiers"):
        ...
        if check_that_files_exist(file_paths):
            try:
                ...
                # Copy files
                copy_old_paths_to_new_paths_from_dictionary(file_paths, new_file_paths, class_mapping=class_mapping)
                num_patient += 1
    ...
    create_dataset_json(dataset_path, modalities, numTraining, class_names=class_names, file_ending=".nii.gz")
